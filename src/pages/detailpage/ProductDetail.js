    ...
    await this.setState({
      rateComponent: this.state.rates.length === 0 ? <></> :
          this.state.rates.map((rate, index) => (<div key={index} className="review d-flex">
            <div className="flex-shrink-0 text-center me-4 me-xl-5"><img className="review-image"
                                                                         src="https://d19m59y37dris4.cloudfront.net/sell/2-0/img/person-1.jpg"
                                                                         alt="Han Solo"/><span
                className="text-uppercase text-muted"></span></div>
            <div>
          ...
          <section className="mt-5">
            <div className="container">
              <ul className="nav nav-tabs flex-column flex-sm-row" role="tablist">
                <li className="nav-item"><a className="nav-link detail-nav-link active" data-bs-toggle="tab"
                                            href="#description" role="tab">Description</a></li>
                <li className="nav-item"><a className="nav-link detail-nav-link" data-bs-toggle="tab"
                                            href="#additional-information" role="tab"
                                            onClick={(e) => this.handleFeatureClick(e)}>Additional
                  Information</a></li>
                <li className="nav-item"><a className="nav-link detail-nav-link" data-bs-toggle="tab"
                                            href="#reviews" role="tab"
                                            onClick={(e) => this.handleRateClick(e)}>Reviews</a></li>
              </ul>
                  ...
                  <div className="row mb-5">
                    <div className="col-lg-10 col-xl-9">
                      ...
                      <FormReview productId={this.props.match.params.id}
                                  postReview={(rate, isCreate) => this.getAllRate(rate, isCreate)}/>
                    </div>
                  </div>
                </div>
              </div>
